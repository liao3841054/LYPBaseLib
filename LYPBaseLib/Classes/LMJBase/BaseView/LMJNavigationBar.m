...
- (void)setupDataSourceUI
{
    ...
    if ([self.dataSource respondsToSelector:@selector(lmjNavigationHeight:)]) {
        ...
        self.lmj_size = CGSizeMake([UIScreen mainScreen].bounds.size.width, [self.dataSource lmjNavigationHeight:self]);
        
    }else
    {
        self.lmj_size = CGSizeMake([UIScreen mainScreen].bounds.size.width, kDefaultNavBarHeight);
    }
