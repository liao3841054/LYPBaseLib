...
{
    NSString *current = [textField.text stringByReplacingCharactersInRange:range withString:string].trim;
    
    if (textField.isEmptyAutoEnable && (LMJIsEmpty(textField.text.trim) || LMJIsEmpty(current))) {
...
- (void)checkIsEmpty:(BOOL)isEmpty textField:(UITextField *)textField
{
    ...
    if (!isEmpty) {
        ...
        [self.requiredTextFields enumerateObjectsUsingBlock:^(UITextField *obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            if (obj != textField && LMJIsEmpty(obj.text.trim)) {
